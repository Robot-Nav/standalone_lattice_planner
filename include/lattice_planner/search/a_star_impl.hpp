// Lattice Planner - A* Search Implementation
// Ported from nav2_smac_planner/a_star_impl.hpp with Node2D branches and ROS deps removed.

#ifndef LATTICE_PLANNER__SEARCH__A_STAR_IMPL_HPP_
#define LATTICE_PLANNER__SEARCH__A_STAR_IMPL_HPP_
  ...
  const unsigned int & dim_3)
{
  ...
  _start = addToGraph(
    getIndex(
      ...
      dim_3));
  _start->setPose(Coordinates(mx, my, static_cast<float>(dim_3)));
}
  ...
  const int & coarse_search_resolution)
{
    ...
    _goal_manager.hasGoalChanged(ref_goal_coord))
  {
    ...
    _shared_ctx->obstacle_heuristic->resetObstacleHeuristic(
      _costmap, _start->pose.x, _start->pose.y, mx, my,
    ...
    case GoalHeadingMode::BIDIRECTIONAL:
      {
        ...
        // Add goal node in opposite (180 deg) direction
...
bool AStarAlgorithm<NodeT>::areInputsValid()
{
  ...
  if (_goal_manager.getGoalsSet().empty()) {
    throw GoalOccupied("Goal was in lethal cost");
  }
  ...
  while (iterations < getMaxIterations() && !_queue.empty()) {
    ...
    if (iterations % _terminal_checking_interval == 0) {
      if (cancel_checker && cancel_checker()) {
        throw PlannerCancelled("Planner was cancelled");
      }
...
#endif  // LATTICE_PLANNER__SEARCH__A_STAR_IMPL_HPP_
