// Lattice Planner - Analytic Expansion Implementation
// Ported from nav2_smac_planner/analytic_expansion_impl.hpp with Node2D branches removed.

#ifndef LATTICE_PLANNER__SEARCH__ANALYTIC_EXPANSION_IMPL_HPP_
#define LATTICE_PLANNER__SEARCH__ANALYTIC_EXPANSION_IMPL_HPP_
...
{
  // Must be a valid motion model for analytic expansion
  ...
  const ompl::base::StateSpacePtr & state_space)
{
  ...
  for (float i = 1; i <= num_intervals; i++) {
    ...
    if (node_getter(index, next)) {
      ...
      proposed_coordinates =
        Coordinates(static_cast<float>(reals[0]), static_cast<float>(reals[1]), angle);
  ...
  if (!failure) {
    ...
    if (max_cost_it != node_costs.end() && *max_cost_it > max_cost) {
      ...
      if (failure) {
        if (d < 2.0f * static_cast<float>(M_PI) * _ctx->motion_table.min_turning_radius &&
  ...
  AnalyticExpansionNodes & analytic_nodes)
{
  ...
  // So, we can attempt to refine the path by increasing the turning radius
  ...
  auto scoringFn = [&](const AnalyticExpansionNodes & expansion) {
      ...
      for (auto iter = expansion.nodes.begin(); iter != expansion.nodes.end(); ++iter) {
        ...
        score += distance * (1.0f + weight * normalized_cost);
      }
      return score;
    };
  ...
  const float max_min_turn_rad = 4.0f * min_turn_rad;  // Up to 4x the turning radius
  ...
  while (min_turn_rad < max_min_turn_rad) {
    min_turn_rad += 0.5f;  // In Grid Coords, 1/2 cell steps
...
{
  // NodeLattice: clear the motion primitive pointer
  node->setMotionPrimitive(nullptr);
}

}  // namespace lattice_planner

#endif  // LATTICE_PLANNER__SEARCH__ANALYTIC_EXPANSION_IMPL_HPP_
